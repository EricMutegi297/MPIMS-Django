...
function UnitDirectoryContent({ units, formations, loading, onCreate, onClose, modal = false }) {
  ...
  const submitUnit = async (event) => {
    ...
    try {
      ...
      setMessage(`${createdName} added successfully.`);
        ...
        {mode === "form" && (
          <form onSubmit={submitUnit} className="mx-auto max-w-3xl space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-2 border-b border-slate-200 pb-4 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-slate-500">Add Unit</p>
...
export function UnitDirectoryModal({ units, formations, loading, onClose, onCreate }) {
  return (
    <div
